...
- **Snapshot:** `versions/v2.8.0/` — **Rollback:** v2.7.1 (`versions/v2.7.1/`)
- **Files:** index.html, sw.js (cache `sales-dash-v35`), manifest.json, icon.svg

### แท็บ YTD: ค่าเฉลี่ยต่อเดือนของ "จำนวนชิ้น" และ "จำนวนบิล"
ผู้ใช้ขอค่าเฉลี่ยของจำนวนชิ้น/จำนวนบิล โดยให้อยู่ในช่องเดียวกับยอดรวม — ทำให้ทั้ง 2 ที่:

- **การ์ด KPI ด้านบน** — การ์ด "จำนวนชิ้น" และ "จำนวนบิล" เพิ่มบรรทัด **"เฉลี่ย X/เดือน"** ใต้ตัวเลขรวม
  (แสดงเฉพาะแท็บ YTD, อยู่เหนือบรรทัดเทียบปีก่อน)
- **ตารางรายสาขา + ตามประเภทร้าน** — ช่อง "ชิ้น" และ "บิล" เพิ่มบรรทัดเล็กใต้ยอดรวมว่า **"⌀ X/เดือน"**
  ทุกแถว (รวมแถว Total และแถวประเภทร้าน) — แสดงเฉพาะแท็บ YTD เช่นกัน
- **ตัวหาร** = จำนวนเดือนที่มียอดขายจริงในช่วง ตัวเดียวกับที่คอลัมน์ "ยอด/เดือน" และการ์ด
  "ยอดขาย/เดือน" ใช้ → ทุกค่า "ต่อเดือน" บนหน้าจอหารด้วยตัวเดียวกันหมด ไม่ขัดกันเอง
- **แท็บอื่นไม่เปลี่ยน** — Daily/MTD/Custom ยังแสดงเฉพาะยอดรวมเหมือนเดิม (ช่วงเหล่านั้นไม่ได้กินหลายเดือน
  ค่าเฉลี่ยต่อเดือนจึงไม่มีความหมาย)
- **การจัดเรียงยังถูกต้อง** — ใส่ `data-sort` เป็นยอดรวมไว้ที่ช่อง ชิ้น/บิล เพราะข้อความในช่องมี 2 บรรทัดแล้ว
  ถ้าไม่ใส่ ตัวเรียงจะอ่านตัวเลขสองชุดปนกัน (คลิกหัวคอลัมน์ยังเรียงตามยอดรวมเหมือนเดิม)

### ทดสอบ (Playwright headless — ข้อมูลจริง ก.ย. โคลนเป็น มิ.ย.–ก.ย. 4 เดือน)
| จุด | ยอดรวม | เฉลี่ย/เดือน | ตรวจ |
|---|---|---|---|
| ตาราง Total — ชิ้น | 160,804 | ⌀ 40,201 | = รวม ÷ 4 ✅ |
| ตาราง Total — บิล | 17,947 | ⌀ 4,487 | = รวม ÷ 4 ✅ |
| การ์ด จำนวนชิ้น | 160,804 | เฉลี่ย 40,201 | ตรงกับตาราง ✅ |
| การ์ด จำนวนบิล | 17,947 | เฉลี่ย 4,487 | ตรงกับตาราง ✅ |

- แท็บ MTD / Daily / Custom: ไม่มีบรรทัดเฉลี่ยโผล่มา ✅
- คลิกหัวคอลัมน์ "ชิ้น" เรียงจากมากไปน้อยตามยอดรวมถูกต้อง (16,480 → 6,464 → 5,768 → …) ✅
- Regression suite เดิม + ชุดทดสอบเป้าหมายรายปี (v2.7.0) ผ่านครบ ไม่มี console error ✅
...
- **Date:** 2026-09-18
- **Status:** ✅ STABLE (ผู้ใช้ยืนยัน "ใช้งานได้" 2026-09-18) — ถูกแทนที่ด้วย v2.8.0 บนเว็บจริงแล้ว (เก็บไว้เผื่อ rollback)
- **Snapshot:** `versions/v2.7.1/` — **Rollback:** v2.7.0 (`versions/v2.7.0/`)
- **Files:** index.html, sw.js (cache `sales-dash-v34`), manifest.json, icon.svg

### แสดงเลขเวอร์ชันบนหน้าเว็บ
- เพิ่มบรรทัด **"Sales Analyst Dashboard • เวอร์ชัน X.Y.Z"** ที่ด้านล่างสุดของหน้า
- แสดงตลอดเวลา ทั้งตอนยังไม่มีข้อมูล (หน้าอัปโหลด) และหลังโหลดข้อมูลแล้ว — คนละบรรทัดกับ footer เดิม
  ที่บอกช่วงข้อมูล/จำนวนแถว (footer เดิมอยู่ใน `#dash` จึงเห็นเฉพาะตอนมีข้อมูล)
- ค่ามาจากตัวแปรเดียว `APP_VERSION` ที่ต้นไฟล์ พร้อมคอมเมนต์กำกับว่าต้องอัปเดตพร้อม changelog + cache
  ของ sw.js ทุกครั้งที่ปล่อยเวอร์ชันใหม่
- **ประโยชน์:** เช็คได้ทันทีว่าเครื่อง/มือถือที่เปิดอยู่ใช้เวอร์ชันไหน ไม่ต้องเดาว่า Service Worker
  ยังแคชของเก่าอยู่หรือเปล่า

### ทดสอบ
- หน้าเปล่า (ยังไม่อัปไฟล์): แสดง "เวอร์ชัน 2.7.1" ✅
- หลังอัปไฟล์: ยังแสดงอยู่ และไม่ทับ footer เดิมที่บอกช่วงข้อมูล ✅
- Regression suite เดิมผ่านครบ ไม่มี console error ✅
...
- **Date:** 2026-09-18
- **Status:** ✅ STABLE (ผู้ใช้ยืนยัน "ใช้งานได้" 2026-09-18) — ถูกแทนที่ด้วย v2.7.1 บนเว็บจริงแล้ว (เก็บไว้เผื่อ rollback)
- **Snapshot:** `versions/v2.7.0/` — **Rollback:** v2.6.1 (`versions/v2.6.1/`)
- **Files:** index.html, sw.js (cache `sales-dash-v33`), manifest.json, icon.svg

### แท็บ YTD: คอลัมน์เป้าหมายเป็น "เป้าทั้งปี (Total Target)"
ผู้ใช้ขอว่าหน้า YTD อยากให้เป้าหมายเป็นรายปี — เดิมคอลัมน์เป้าหมาย/%เป้าหมาย ขึ้น "—" ทุกแท็บยกเว้น MTD

- **หัวคอลัมน์เปลี่ยนตามแท็บ** (แบบเดียวกับ ยอด/วัน ↔ ยอด/เดือน และ พยากรณ์สิ้นเดือน ↔ สิ้นปี):
  MTD = **"เป้าหมาย (เดือนนี้)"**, YTD = **"เป้าหมาย (ทั้งปี)"**, Daily/Custom ยังเป็น "—"
- **ค่าเป้าทั้งปี** = ผลรวมเป้ารายเดือน **ม.ค.–ธ.ค.** ของสาขานั้น (Total Target) — แถวประเภทร้านและแถว
  Total รวมขึ้นมาจากรายสาขาเหมือนเดิม จึงยังบวกกันได้ลงตัว
- **% เป้าหมาย** = ยอดขายสะสม YTD ÷ เป้าทั้งปี (ตัวเลขตรงไปตรงมา ไม่ปรับแต่ง)

### เกณฑ์สีของ % เป้าหมายในแท็บ YTD (ต่างจาก MTD โดยตั้งใจ)
- **MTD (เหมือนเดิม):** ≥100% เขียว · 80–99% เหลือง · <80% แดง — เทียบกับเป้าทั้งเดือน
- **YTD (ใหม่):** เทียบกับ **สัดส่วนเดือนที่มีข้อมูล** ไม่ใช่ 100% ของทั้งปี
  → มีข้อมูล 4/12 เดือน = คาดหวัง 33.3% ของเป้าทั้งปี • ทำได้ ≥33.3% (100% ของแผน) = เขียว ·
  ≥90% ของแผน = เหลือง · ต่ำกว่านั้น = แดง
- **เหตุผล:** ถ้าใช้เกณฑ์เดิม เดือน ก.ย. ที่ทำได้ 70% ของเป้าทั้งปี (ซึ่งคือ "เกินแผน") จะขึ้นแดงทั้งตาราง
  ทุกเดือนจนถึงสิ้นปี = สีไม่มีความหมาย ตัวเลข % ยังเป็นค่าจริงเสมอ เปลี่ยนเฉพาะ "สีบอกว่าตามแผนไหม"
- คำอธิบายเกณฑ์อยู่ใต้ตารางและในบรรทัดสรุปเหนือรายงานแล้ว

### ทดสอบ (Playwright headless — 116 สาขา × เป้า ฿1,000,000/เดือน × 12 เดือน, ข้อมูลจริง 4 เดือน)
| แท็บ | หัวคอลัมน์ | เป้า (Total) | % |
|---|---|---|---|
| YTD | เป้าหมาย (ทั้งปี) | ฿1,392,000,000 (= 116 × 12 ล้าน) ✅ | 18.0% = ยอดสะสม ÷ เป้าทั้งปี ✅ |
| MTD | เป้าหมาย (เดือนนี้) | ฿116,000,000 (ไม่เปลี่ยน) ✅ | 54.0% ✅ |
| Daily/Custom | เป้าหมาย (เดือนนี้) | "—" (ไม่เปลี่ยน) ✅ | "—" ✅ |

- เป้ารายสาขาในแท็บ YTD = ฿12,000,000 (12 เดือน × 1 ล้าน) ✅
- เกณฑ์สี: เกินแผน 200% → เขียว · ตามแผนพอดี 100% → เขียว · 93% ของแผน → เหลือง · 60% ของแผน → แดง ✅
- Regression suite เดิม + ชุดทดสอบพยากรณ์สิ้นปี (v2.6.0) ผ่านครบ ไม่มี console error ✅

### หมายเหตุ
- การ์ด Sales Health "ยอดขาย vs เป้าหมาย" ในแท็บ YTD ยังเทียบกับ **เป้าสะสม ม.ค.–เดือนปัจจุบัน**
  (ไม่ใช่เป้าทั้งปี) ซึ่งเป็นค่าที่ถูกต้องสำหรับคำถาม "ถึงตอนนี้ทำได้ตามเป้าไหม" — คนละคำถามกับคอลัมน์
  "เป้าหมาย (ทั้งปี)" ในตาราง จึงตั้งใจไม่เปลี่ยน
...
- **Date:** 2026-09-18
- **Status:** ✅ STABLE (ผู้ใช้ยืนยัน "ใช้ได้แล้ว" บนมือถือจริง 2026-09-18) — ถูกแทนที่ด้วย v2.7.1 บนเว็บจริงแล้ว (เก็บไว้เผื่อ rollback)
...
- **Date:** 2026-09-18
- **Status:** ✅ STABLE (ผู้ใช้ยืนยัน "ใช้ได้" 2026-09-18) — ถูกแทนที่ด้วย v2.6.1 บนเว็บจริงแล้ว (เก็บไว้เผื่อ rollback)
